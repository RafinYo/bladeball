...
local Tabs = {
    Info = Window:AddTab({ Title = "Information", Icon = "newspaper" }),
    Combat = Window:AddTab({ Title = "Parry", Icon = "shield-check" }),
...
function initializate(dataFolder_name: string)
    ...
    hit_Sound.SoundId = 'rbxassetid://6729922069'

    hit_Sound.Volume = 0.50
...
local Gays = Tabs.Info:AddSection("Version")

Gays:AddParagraph({
        Title = "Version",
        Content = "Version v 3.0.0 re-continued Exploit X from version 2.9.5"
    })

local Gayss = Tabs.Info:AddSection("Update log")

Gayss:AddParagraph({
        Title = "log 1",
        Content = "Changed from beta to premium 3.0.0 not paid."
    })

local Gyatt = Tabs.Combat:AddSection("announcement")

Gyatt:AddParagraph({
        Title = "Rafin",
        Content = "WE ARE BACK Boys!!!!!!!!!!"
    })


local Mainy = Tabs.Combat:AddSection("Automation")
...
local Toggle = Mainy:AddToggle("Auto spam", {Title = "Auto spam", Default = false })
Toggle:OnChanged(function(toggled)
...
local Gyott = Tabs.Combat:AddSection("Ball")

local Toggle = Gyott:AddToggle("View Visual", {Title = "View Part", Default = false })
Toggle:OnChanged(function(toggled)
        if toggled then 
            local globals = {}
            local TweenService = game:GetService("TweenService")
...
local Toggle = Gyott:AddToggle("look at ball", {Title = "Look at Ball", Default = false })
Toggle:OnChanged(function(toggled)
...
local Guys = Tabs.Combat:AddSection("Effects")


local Toggle = Guys:AddToggle("Hit Sound", {Title = "Hit sound", Default = false })
Toggle:OnChanged(function(toggled)
...
local Toggle = Guys:AddToggle("hit effect", {Title = "hit effect", Default = false })
Toggle:OnChanged(function(toggled)
...
--misc
local Misc = Tabs.Misc:AddSection("manual (crates)")
...
local Moneys = Tabs.Misc:AddSection("Automation (crates")


local Toggle = Moneys:AddToggle("SwordAuto", {Title = "Auto Open Sword Crate", Default = false })
Toggle:OnChanged(function(Value)
_G.AutoSword = Value
        SwordCrateAuto()
end)

local Toggle = Moneys:AddToggle("ExplosionAuto", {Title = "Auto Open Explosion Crate", Default = false })
Toggle:OnChanged(function(Value)
_G.AutoBoom = Value
        ExplosionCrateAuto()
end)

local Mc = Tabs.Misc:AddSection("Esp")
...
local Mc = Tabs.Misc:AddSection("Server")
...
local AAA = Tabs.Misc:AddSection("Player")

local Slider = AAA:AddSlider("Slider", {
        Title = "Walk speed",
        Description = "",
        Default = 21,
        Min = 0,
...
--settings
local Settings = Tabs.Misc:AddSection("Settings")
...
Credits:AddParagraph({
        Title = "Credits",
        Content = "Script Made by Xyris Hub all rights own it"
    })

Credits:AddParagraph({
        Title = "Developers",
        Content = "Xyris Hub Team!"
    })
